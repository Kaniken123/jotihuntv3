...
const storage = multer_1.default.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, 'hunt-' + uniqueSuffix + path_1.default.extname(file.originalname));
    }
});
...
const calculateHuntPoints = (huntArea, teamArea) => {
...
const checkHuntCooldown = async (userId, foxArea) => {
    const cooldownTime = new Date(Date.now() - HUNT_COOLDOWN_MINUTES * 60 * 1000);
    const recentHunt = await (0, database_1.db)('hunts')
...
router.get('/stats', auth_1.authenticateToken, auth_1.enforceTenantIsolation, async (req, res) => {
    try {
        ...
        const stats = await statsQuery
            .select(database_1.db.raw('COUNT(*) as total_hunts'), database_1.db.raw('COUNT(CASE WHEN status = "approved" THEN 1 END) as approved_hunts'), database_1.db.raw('COUNT(CASE WHEN status = "rejected" THEN 1 END) as rejected_hunts'), database_1.db.raw('COUNT(CASE WHEN status = "pending" THEN 1 END) as pending_hunts'), database_1.db.raw('SUM(CASE WHEN status = "approved" THEN points_awarded ELSE 0 END) as total_points'))
            .first();
        res.json(stats);
    }
    catch (error) {
