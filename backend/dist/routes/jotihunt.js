...
router.get('/areas', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/articles', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (area) {
            query = query.where(function () {
                this.where('area', area).orWhereNull('area');
            });
        }
