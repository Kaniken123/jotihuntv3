...
router.post('/solutions', authenticateToken, async (req, res) => {
  try {
    ...
    const existingSolution = await db('hint_solutions')
    ...
    // Insert solution
    const [solution_id] = await db('hint_solutions').insert({
    ...
    if (reveals_fox_location) {
      for (const areaName of revealedAreas) {
        ...
        if (coordinateData[`${areaLower}_lat`] && coordinateData[`${areaLower}_lng`]) {
          await revealFoxLocation(
            areaName, 
            coordinateData[`${areaLower}_lat`], 
            coordinateData[`${areaLower}_lng`], 
            user_id
          );
        }
      }
    ...
    const createdSolution = await db('hint_solutions')
